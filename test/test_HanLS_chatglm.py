...
from glmtuner import get_train_args
...
output_dir = '../output/predict_chatglm_ria.json' if use_ria else '../output/predict_chatglm.json'
with open(output_dir,'w',encoding='utf-8') as wf:
    for data, predict in zip(test_data, predict_sentences):
        data.update({'answer':predict, 'model':'ChatGLM_RIA' if use_ria else 'ChatGLM'})
